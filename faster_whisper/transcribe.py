...
try:
    ...
    # Delay import-time failure for optional dependencies (e.g., requests) and
    # provide fallbacks that raise informative errors when used.
    def _missing_import_error(*args, **kwargs):
        raise ModuleNotFoundError(
            "Optional dependency for 'faster_whisper.utils' is missing. "
            "Install 'requests' or the package extras (e.g., pip install -e .[dev]) "
            "to enable this functionality."
        )

    download_model = _missing_import_error
    format_timestamp = _missing_import_error
    get_end = _missing_import_error

    def get_logger(name=None):
        # provide a basic logger to avoid failures when only logging is needed
        return logging.getLogger(name or "faster-whisper")
